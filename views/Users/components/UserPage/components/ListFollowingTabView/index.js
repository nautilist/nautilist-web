...
function view(state, emit){
    const {listsFollowing} = state.main.selected.user;

    if(!Object.keys(listsFollowing).length > 0){
    ...
    <section class="${styles.sectionmw7} pa2 bn bg-white mt3 mb5">
        ...
        <ul class="list pl0">
            ${ListCards(listsFollowing.data, state, emit)}
        </ul>
    </section>
    `
}
...
function ListCard(list, state, emit){
    ...
    <li onclick=${navigateTo}
        class="w-100 ba bw1 mt2 mb2 pa3-ns pa2 bg-lightest-blue dropshadow flex flex-column flex-row-ns justify-between-ns pointer">
        <div class="w-75-ns w-100">
        <h2 class="pa0 ma0 f4 f5-ns">${name}</h2>
        <p class="pa0 ma0 f7 f6-ns">by ${ownerDetails.username}</p>
